...
public class GraphCanvas extends AnchorPane {
    ...
    private Pane content = new Pane(); // Nội dung của PopOver

    private void initEvent() {
        this.setOnMousePressed(ob -> {
            notifyMousePressed(ob);

            int graphIndex = selectionMatrix.GetNearbyGraphIndex((int) ob.getX(), (int) ob.getY());
            mnr.setSelectedGraph(graphIndex);
            if (graphIndex == -1) {
